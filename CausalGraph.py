...
class CausalGraph:
    def __init__(self, data, edges, latent_edges):
        """
        data : {Node : array of length N}
        edges : List of pair of tuples, i.e., [(edge from Node A, edge to Node B)]
        latent_edges : List of pair of tuples, i.e., [(edge from Node A, edge to Node B)]; information about the confounding structure. If not given, 
        we asuume full confounding 
        """
        self.data = data
        self.edges = edges
        self.latent_edges = latent_edges
        self.dag = nx.DiGraph()
        self.nodes = list(self.data.keys()) # List of nodes in the graph; endogeneous variables 
        ...
        self.latent_nodes = ['U_{}'.format(i) for i in self.observed_index]  # exogeneous variables 
        #self.rf_nodes = ['R_{}'.format(i) for i in self.observed_index] 
        self.latent_index = [self.latent_nodes.index(lat_node)+1 for 
                             lat_node in self.latent_nodes]
        ...
        self.data_nd = np.array([list(self.data.values())[i] for i in range(len(self.nodes))]) # Converting dictionary to arrays
        ...
        # Adding confounding structure to the graph
        self.conf_edges = []
    ...
    def draw(self):
        """
        Draws a causal graph with given confounding structure
        """
